...
BEGIN
    ...
    FOR usuario IN
        ...
        FROM (
            ...
            FROM mdl_user
            WHERE
            username NOT IN (SELECT u.username
                            FROM mdl_cohort_members mcm
                                INNER JOIN mdl_user u  ON mcm.userid = u.id
                                INNER JOIN mdl_cohort mc ON mc.id = mcm.cohortid
                            WHERE mc.contextid = 934651) ) AS users_to_modify

        WHERE users_to_modify.id <> 1
...
BEGIN
    ...
    FOR object IN
        SELECT b.id
        FROM mdl_book b
        JOIN mdl_course c ON b.course = c.id
        WHERE c.id NOT IN (SELECT c.id
                            FROM mdl_course c
                                INNER JOIN mdl_course_categories cc ON cc.id = c.category
                            WHERE
...
BEGIN
    ...
    FOR object IN
        SELECT ch.id
        FROM mdl_chat ch
        JOIN mdl_course c ON ch.course = c.id
        WHERE c.id NOT IN (SELECT c.id
                            FROM mdl_course c
                                INNER JOIN mdl_course_categories cc ON cc.id = c.category
                            WHERE
...
BEGIN
    ...
    FOR object IN
        SELECT h.id
        FROM mdl_hotpot h
        JOIN mdl_course c ON h.course = c.id
        WHERE c.id NOT IN (SELECT c.id
                            FROM mdl_course c
                                INNER JOIN mdl_course_categories cc ON cc.id = c.category
                            WHERE
...
BEGIN
    ...
    FOR object IN
        SELECT j.id
        FROM mdl_journal j
        JOIN mdl_course c ON j.course = c.id
        WHERE c.id NOT IN (SELECT c.id
                            FROM mdl_course c
                                INNER JOIN mdl_course_categories cc ON cc.id = c.category
                            WHERE
...
BEGIN
    ...
    FOR object IN
        SELECT p.id
        FROM mdl_page p
        JOIN mdl_course c ON p.course = c.id
        WHERE c.id NOT IN (SELECT c.id
                            FROM mdl_course c
                                INNER JOIN mdl_course_categories cc ON cc.id = c.category
                            WHERE
...
BEGIN
    ...
    FOR object IN
        SELECT r.id
        FROM mdl_resource r
        JOIN mdl_course c ON r.course = c.id
        WHERE c.id NOT IN (SELECT c.id
                            FROM mdl_course c
                                INNER JOIN mdl_course_categories cc ON cc.id = c.category
                            WHERE
...
BEGIN
    ...
    FOR object IN
        SELECT u.id
        FROM mdl_url u
        JOIN mdl_course c ON u.course = c.id
        WHERE c.id NOT IN (SELECT c.id
                            FROM mdl_course c
                                INNER JOIN mdl_course_categories cc ON cc.id = c.category
                            WHERE
...
DROP FUNCTION IF EXISTS updateCourses();
